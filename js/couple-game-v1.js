...
class CoupleGame {
    ...
    saveCompletedTasks() {
        ...
        window.setData && window.setData('game_completed_tasks', this.completedTasks);
        // 推送到 D1 云端
        if (this.completedTasks.length > 0 && window.CloudSync) {
            window.CloudSync.saveScore(this.completedTasks[this.completedTasks.length - 1]);
        }
    }
}
