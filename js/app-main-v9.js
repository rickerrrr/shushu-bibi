...
async function doLogin(user) {
  ...
  const other = currentUser === 'shushu' ? 'bibi' : 'shushu';
  const myName = currentUser === 'shushu' ? '鼠鼠' : '笔笔';
  const otherName = currentUser === 'shushu' ? '笔笔' : '鼠鼠';

  // 禁用所有头像
  const avatars = document.getElementById('login-avatars');
  if (avatars) avatars.classList.add('all-disabled');
  ...
  // Persist current user to localStorage (both keys for backward compat)
  localStorage.setItem('currentUser', currentUser);
  localStorage.setItem('current_user', currentUser);

  // 🔥 同步到 realtime-sync-v6 的内存状态（解决笔笔登录显示鼠鼠的致命bug）
  if (window.switchUser && typeof window.switchUser === 'function') {
    window.switchUser(currentUser);
  } else {
    // 兜底：如果 realtime-sync 还没加载，延迟重试
    var _fixUser = currentUser;
    setTimeout(function() {
      if (window.switchUser && typeof window.switchUser === 'function') {
        window.switchUser(_fixUser);
      }
    }, 2000);
  }
  console.log('[doLogin] 已同步身份到 realtime-sync:', currentUser);
  ...
  const msgList = messages[other] || messages.shushu;
  ...
  if (bubbleWrapper && bubble) {
    bubble.className = 'chat-bubble typing ' + other;
    bubbleWrapper.classList.add('show');
...
function triggerSolarTerm() {
  const term = getCurrentSolarTerm();
  if (!term) return;

  // 规则：每个节气只播放一次（key = 节气名 + 日期）
  const today = new Date().toISOString().slice(0, 10);
  const storageKey = 'lb_st_' + term.name;
  const lastShown = localStorage.getItem(storageKey);
  
  if (lastShown === today) {
    return; // 该节气今天已播放过，等下一个节气
  }
  ...
  // 延迟触发，等页面加载完
  setTimeout(() => startSolarTermAnimation(term), 600);
}
...
const SOLAR_TERMS = [
  { name:'小寒', m:1, d:6,  desc:'冷气积久而为寒', sub:'Minor Cold',   season:'winter' },
  { name:'大寒', m:1, d:20, desc:'寒气之逆极',     sub:'Major Cold',   season:'winter' },
  { name:'立春', m:2, d:4,  desc:'东风解冻，万物复苏', sub:'Spring Begins',  season:'spring' },
  { name:'雨水', m:2, d:19, desc:'春雨润物细无声', sub:'Rain Water',     season:'spring' },
  { name:'惊蛰', m:3, d:6,  desc:'春雷乍动，惊醒蛰虫', sub:'Awakening of Insects', season:'spring' },
  { name:'春分', m:3, d:21, desc:'昼夜均而寒暑平', sub:'Spring Equinox',  season:'spring' },
  { name:'清明', m:4, d:5,  desc:'气清景明，万物皆显', sub:'Clear and Bright', season:'spring' },
  { name:'谷雨', m:4, d:20, desc:'雨生百谷',       sub:'Grain Rain',     season:'spring' },
  { name:'立夏', m:5, d:5,  desc:'天地始交，万物并秀', sub:'Summer Begins',  season:'summer' },
  { name:'小满', m:5, d:21, desc:'物致于此小得盈满', sub:'Grain Buds',     season:'summer' },
  { name:'芒种', m:6, d:6,  desc:'有芒之谷可播种', sub:'Grain in Ear',   season:'summer' },
  { name:'夏至', m:6, d:21, desc:'日长之至，万物繁茂', sub:'Summer Solstice',  season:'summer' },
  { name:'小暑', m:7, d:7,  desc:'暑，热也，就热之中分为大小', sub:'Minor Heat',  season:'summer' },
  { name:'大暑', m:7, d:22, desc:'湿热交蒸，郁蒸至极', sub:'Major Heat',  season:'summer' },
  { name:'立秋', m:8, d:7,  desc:'一叶梧桐一报秋', sub:'Autumn Begins',  season:'autumn' },
  { name:'处暑', m:8, d:23, desc:'暑气至此而止',   sub:'End of Heat',    season:'autumn' },
  { name:'白露', m:9, d:8,  desc:'露凝而白也',     sub:'White Dew',      season:'autumn' },
  { name:'秋分', m:9, d:23, desc:'昼夜均，寒暑平', sub:'Autumn Equinox',  season:'autumn' },
  { name:'寒露', m:10,d:8,  desc:'露气寒冷，将凝结也', sub:'Cold Dew',    season:'autumn' },
  { name:'霜降', m:10,d:23, desc:'气肃而凝，露结为霜', sub:'Frost Descent', season:'autumn' },
  { name:'立冬', m:11,d:7,  desc:'水始冰，地始冻', sub:'Winter Begins',  season:'winter' },
  { name:'小雪', m:11,d:22, desc:'雨下而为寒气所薄', sub:'Minor Snow',   season:'winter' },
  { name:'大雪', m:12,d:7,  desc:'至此而雪盛也',   sub:'Major Snow',     season:'winter' },
  { name:'冬至', m:12,d:22, desc:'日短之至，阳气始生', sub:'Winter Solstice', season:'winter' },
];
...
function getCurrentSolarTerm() {
  ...
  // 如果1月1日到小寒之间，属于大寒（上一年的最后一个节气）
  return SOLAR_TERMS[SOLAR_TERMS.length - 1];
}

// Canvas 粒子系统
let solarAnimRaf = null;
let solarParticles = [];
let solarFrame = 0;

const SOLAR_THEME = {
  summer: {
    bg: ['#0c3d5c', '#084c6e', '#006994'],
    glow: ['rgba(56,189,248,0.25)', 'rgba(34,211,238,0.2)', 'rgba(14,165,233,0.18)'],
    emojis: ['🌊', '🐠', '🐚', '🫧', '🐋', '🐬', '🦀', '🧜‍♀️', '💙', '🏄', '🐟', '🦈', '⛵', '🪸'],
    sizes: [22, 50],
    speed: [0.3, 2.0],
    ocean: true,
  ...
  winter: {
    bg: ['#ede9fe', '#e0f2fe', '#f0f4f8'],
    glow: ['rgba(129,140,248,0.15)', 'rgba(56,189,248,0.12)', 'rgba(168,85,247,0.12)'],
    emojis: ['❄️', '⛄', '🌟', '🎄', '🕯️', '🦌', '❄️', '🌨️'],
    sizes: [16, 42],
    speed: [0.5, 3.0],
  },
  spring: {
    bg: ['#eff6ff', '#f0fdf4', '#fefce8'],
    glow: ['rgba(96,165,250,0.15)', 'rgba(74,222,128,0.12)', 'rgba(250,204,21,0.12)'],
    emojis: ['🌸', '🌺', '🦋', '🐦', '🌱', '🌷', '💮', '🕊️'],
    sizes: [18, 44],
    speed: [0.3, 2.5],
  },
  autumn: {
    bg: ['#fff7ed', '#fefce8', '#eff6ff'],
    glow: ['rgba(249,115,22,0.15)', 'rgba(250,204,21,0.12)', 'rgba(220,38,38,0.12)'],
    emojis: ['🍂', '🍁', '🌾', '🍃', '🐿️', '🦊', '🍄', '🦉'],
    sizes: [18, 46],
    speed: [0.3, 2.5],
  },
...
class SolarParticle {
  ...
  reset() {
    const t = this.theme;
    this.emoji = t.emojis[Math.floor(Math.random() * t.emojis.length)];
    this.x = Math.random() * this.cvs.width;
    this.y = this.cvs.height + 60;
    this.size = t.sizes[0] + Math.random() * (t.sizes[1] - t.sizes[0]);
    this.speed = t.speed[0] + Math.random() * (t.speed[1] - t.speed[0]);
    this.wobble = Math.random() * Math.PI * 2;
    this.wobbleSpeed = 0.02 + Math.random() * 0.06;
...
function startSolarTermAnimation(term) {
  const overlay = document.getElementById('solar-term-overlay');
  const canvas = document.getElementById('solar-term-canvas');
  if (!overlay || !canvas) return;

  // Set season theme class for 3D text coloring
  const stage = overlay.querySelector('.solar-term-stage');
  if (stage) {
    stage.className = 'solar-term-stage solar-term-' + term.season;
  }

  // 更新文字
  const textEl = document.getElementById('solar-term-text');
  const subEl = document.getElementById('solar-term-sub');
  const descEl = document.getElementById('solar-term-desc');
  if (textEl) textEl.textContent = term.name;
  if (subEl) subEl.textContent = term.sub;
  if (descEl) descEl.textContent = term.desc;
  ...
  const theme = SOLAR_THEME[term.season] || SOLAR_THEME.summer;
  const ctx = canvas.getContext('2d');

  // Create particles
  solarParticles = [];
  const count = Math.min(25, Math.floor(canvas.width / 40));
  for (let i = 0; i < count; i++) {
    solarParticles.push(new SolarParticle(canvas, theme));
  }

  const isOcean = theme.ocean === true;

  // Firefly particles - color depends on season
  const fireflies = [];
  for (let i = 0; i < 15; i++) {
    fireflies.push({
  ...
  // Bubble particles for ocean theme
  const bubbles = [];
  if (isOcean) {
    for (let i = 0; i < 30; i++) {
      bubbles.push({
  ...
  function draw() {
    ...
    // Background gradient
    bgChangeTimer++;
    if (bgChangeTimer > 200) {
      bgChangeTimer = 0;
      bgTarget = bgTarget === 0 ? 1 : 0;
    }
    bgAlpha += (bgTarget - bgAlpha) * 0.01;

    if (isOcean) {
      // Ocean: deep blue → turquoise linear gradient top to bottom
      const bgGrad = ctx.createLinearGradient(0, 0, 0, canvas.height);
      bgGrad.addColorStop(0, '#021a2e');
      bgGrad.addColorStop(0.3, '#063550');
      bgGrad.addColorStop(0.6, '#08608a');
    ...
    glowGrad.addColorStop(0, theme.glow[0]);
    glowGrad.addColorStop(1, 'transparent');
    ctx.fillStyle = glowGrad;
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    // === Ocean Waves (summer only) ===
    if (isOcean) {
      wavePhase += 0.015;
      ...
      for (let wl = 0; wl < waveCount; wl++) {
        const offset = wl * 30;
        const speed = 1 + wl * 0.5;
        const amp = 8 + wl * 6;
        const freq = 0.008 - wl * 0.002;

        ctx.beginPath();
        ctx.moveTo(0, canvas.height);
        for (let x = 0; x <= canvas.width; x += 3) {
          const y = waveY - offset + Math.sin(x * freq + wavePhase * speed + wl) * amp;
          ctx.lineTo(x, y);
        }
        ctx.lineTo(canvas.width, canvas.height);
        ctx.closePath();

        const waveGrad = ctx.createLinearGradient(0, waveY - offset - amp, 0, canvas.height);
        const baseAlpha = 0.4 - wl * 0.1;
        waveGrad.addColorStop(0, `rgba(56,189,248,${baseAlpha})`);
        waveGrad.addColorStop(0.5, `rgba(14,165,233,${baseAlpha * 0.7})`);
        waveGrad.addColorStop(1, 'rgba(2,26,46,0.3)');
      ...
      splashes.forEach(sp => {
        ...
        if (sp.life <= 0 || sp.y < 0) {
          ...
          sp.y = canvas.height * (0.65 + Math.random() * 0.35);
          sp.vy = -1 - Math.random() * 3;
          sp.vx = (Math.random() - 0.5) * 2;
          sp.life = 1;
        }
        ctx.fillStyle = `rgba(224,242,254,${sp.life * 0.9})`;
        ctx.beginPath();
    ...
    // Draw fireflies (season-appropriate colors)
    fireflies.forEach(ff => {
      ff.phase += ff.speed;
      ff.alpha = 0.3 + Math.sin(ff.phase) * 0.4 + 0.4;
      ff.x += Math.cos(ff.phase * 2) * 0.5;
      ff.y += Math.sin(ff.phase * 3) * 0.3;
      ...
      const fGrad = ctx.createRadialGradient(ff.x, ff.y, 0, ff.x, ff.y, ff.r * 3);
      if (isOcean) {
        fGrad.addColorStop(0, `rgba(34,211,238,${ff.alpha})`);
        fGrad.addColorStop(0.5, `rgba(6,182,212,${ff.alpha * 0.4})`);
      } else {
        fGrad.addColorStop(0, `rgba(253,230,138,${ff.alpha})`);
        fGrad.addColorStop(0.5, `rgba(252,211,77,${ff.alpha * 0.4})`);
      }
      ...
      ctx.fillStyle = isOcean ? `rgba(207,250,254,${ff.alpha})` : `rgba(254,243,199,${ff.alpha})`;
      ctx.beginPath();
    ...
    if (isOcean) {
      bubbles.forEach(b => {
        ...
        bGrad.addColorStop(0.4, 'rgba(186,230,253,0.3)');
        bGrad.addColorStop(1, 'rgba(14,165,233,0.15)');
    ...
    solarParticles.forEach(p => {
      p.update();
      p.draw(ctx, solarFrame);
    });

    solarFrame++;
    solarAnimRaf = requestAnimationFrame(draw);
  }
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      cancelAnimationFrame(solarAnimRaf);
